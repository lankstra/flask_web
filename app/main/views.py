...
def index():
    ...
    pagination = query.order_by(Post.timestamp.desc()).paginate(page,
                                                                per_page=current_app.config['UR2B_POSTS_PER_PAGE'],
                                                                error_out=False)
    posts = pagination.items
    return render_template('index.html', form=form, posts=posts, show_brief=True,
                           pagination=pagination, show_followed=show_followed, current_time=datetime.utcnow())
...
def user(username):
    ...
    return render_template('user.html', user=user, posts=posts, show_brief=True)
...
def post(id):
    ...
    page = request.args.get('page', 1, type=int)
    if page == -1:
        page = (post.comments.count() - 1) / current_app.config['UR2B_COMMENTS_PER_PAGE'] + 1
    pagination = post.comments.order_by(Comment.timestamp.asc()) \
        .paginate(page, per_page=current_app.config['UR2B_COMMENTS_PER_PAGE'], error_out=False)
...
def followers(username):
    ...
    pagination = user.followers.paginate(
        page, per_page=current_app.config['UR2B_FOLLOWERS_PER_PAGE'],
        error_out=False)
