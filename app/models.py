...
import misaka as m
from .pygments import HighlighterRenderer
...
class User(UserMixin, db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.role is None:
            if self.email == current_app.config['UR2B_ADMIN']:
...
class Comment(db.Model):
    ...
    def on_change_body(target, value, old_value, initiator):
        ...
        target.body_html = bleach.linkify(
            bleach.clean(markdown(value, output_file='html'), tags=allow_tags, strip=True))
